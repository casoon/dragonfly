...
(function() {
  ...
  function toggleHighContrast(enable) {
    if (typeof enable !== 'boolean') {
      ...
      enable = !document.documentElement.classList.contains(HIGH_CONTRAST_CLASS);
    }
    ...
    try {
      localStorage.setItem(STORAGE_KEY_CONTRAST, enable ? 'true' : 'false');
    } catch (error) {
  ...
  function toggleReducedMotion(enable) {
    if (typeof enable !== 'boolean') {
      // Toggle-Modus, wenn kein Parameter übergeben wird
      enable = !document.documentElement.classList.contains(REDUCED_MOTION_CLASS);
    }
    ...
    // Speichern der Einstellung
    try {
      localStorage.setItem(STORAGE_KEY_MOTION, enable ? 'true' : 'false');
    } catch (error) {
  ...
  function ensureKeyboardAccessibility() {
    ...
    document.querySelectorAll('.clickable:not([tabindex]), .keyboard-focusable:not([tabindex])').forEach(element => {
      element.setAttribute('tabindex', '0');
    });
  }
